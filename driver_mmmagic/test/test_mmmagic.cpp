...
typedef void (*MM_SETMMOUTPUTREPORTS)(unsigned char, unsigned char,
...
int main() {
  ...
  mm_connect_minimaid =
      (MM_CONNECT_MINIMAID)GetProcAddress(hModule, "mm_connect_minimaid");
  mm_setKB = (MM_SETKB)GetProcAddress(hModule, "mm_setKB");
  mm_setDDRPad1Light =
      (MM_SETDDRPAD1LIGHT)GetProcAddress(hModule, "mm_setDDRPad1Light");
  mm_setDDRPad2Light =
      (MM_SETDDRPAD2LIGHT)GetProcAddress(hModule, "mm_setDDRPad2Light");
  mm_setDDRCabinetLight =
      (MM_SETCABINETLIGHT)GetProcAddress(hModule, "mm_setDDRCabinetLight");
  mm_setDDRBassLight =
      (MM_SETDDRBASSLIGHT)GetProcAddress(hModule, "mm_setDDRBassLight");
  ...
  mm_setMMOutputReports =
      (MM_SETMMOUTPUTREPORTS)GetProcAddress(hModule, "mm_setMMOutputReports");
  mm_sendDDRMiniMaidUpdate = (MM_SENDDDRMINIMAIDUPDATE)GetProcAddress(
      hModule, "mm_sendDDRMiniMaidUpdate");

  if (!mm_connect_minimaid || !mm_setKB || !mm_setDDRPad1Light ||
      !mm_setDDRCabinetLight || !mm_setDDRBassLight || !mm_setDDRPad2Light ||
      !mm_setDDRAllOn || !mm_setDDRAllOff || !mm_setBlueLED ||
      !mm_setMMOutputReports || !mm_sendDDRMiniMaidUpdate) {
    const DWORD error =
