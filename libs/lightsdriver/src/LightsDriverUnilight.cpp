...
LightsDriverUnilight::LightsDriverUnilight() {
  hidHelper =
      new HIDHelper(BD_UNILIGHT_VID, BD_UNILIGHT_PID, BD_UNILIGHT_INTERFACE);
};
...
void LightsDriverUnilight::Disconnect() { hidHelper->Close(); }
...
void LightsDriverUnilight::Set(const LightsState *ls) {
  ...
  cabLights |=
      ls->marquee_up_left ? BD_UNILIGHT_LIGHT_CAB_MARQUEE_UP_LEFT : 0x00;
  cabLights |=
      ls->marquee_up_right ? BD_UNILIGHT_LIGHT_CAB_MARQUEE_UP_RIGHT : 0x00;
  cabLights |=
      ls->marquee_lr_left ? BD_UNILIGHT_LIGHT_CAB_MARQUEE_LR_LEFT : 0x00;
  cabLights |=
      ls->marquee_lr_right ? BD_UNILIGHT_LIGHT_CAB_MARQUEE_LR_RIGHT : 0x00;
