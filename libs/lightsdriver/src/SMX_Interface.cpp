...
bool SMX_Interface::isDllLoaded()
{
	return dllLoaded;
}
...
bool SMX_Interface::LoadSMXDLL(const char *dllPath)
{
	...
	SMX_SetLogCallback_ptr = (SMX_SetLogCallback_t)GetProcAddress(hModule, "SMX_SetLogCallback");
	SMX_GetInfo_ptr = (SMX_GetInfo_t)GetProcAddress(hModule, "SMX_GetInfo");
	SMX_GetInputState_ptr = (SMX_GetInputState_t)GetProcAddress(hModule, "SMX_GetInputState");
	SMX_SetLights_ptr = (SMX_SetLights_t)GetProcAddress(hModule, "SMX_SetLights");
	SMX_SetLights2_ptr = (SMX_SetLights2_t)GetProcAddress(hModule, "SMX_SetLights2");
	SMX_ReenableAutoLights_ptr = (SMX_ReenableAutoLights_t)GetProcAddress(hModule, "SMX_ReenableAutoLights");
	...
	SMX_FactoryReset_ptr = (SMX_FactoryReset_t)GetProcAddress(hModule, "SMX_FactoryReset");
	SMX_ForceRecalibration_ptr = (SMX_ForceRecalibration_t)GetProcAddress(hModule, "SMX_ForceRecalibration");
	SMX_SetTestMode_ptr = (SMX_SetTestMode_t)GetProcAddress(hModule, "SMX_SetTestMode");
	SMX_GetTestData_ptr = (SMX_GetTestData_t)GetProcAddress(hModule, "SMX_GetTestData");
	SMX_SetPanelTestMode_ptr = (SMX_SetPanelTestMode_t)GetProcAddress(hModule, "SMX_SetPanelTestMode");
	SMX_Version_ptr = (SMX_Version_t)GetProcAddress(hModule, "SMX_Version");

	if (!SMX_Start_ptr || !SMX_Stop_ptr || !SMX_SetLogCallback_ptr || !SMX_GetInfo_ptr ||
		!SMX_GetInputState_ptr || !SMX_SetLights_ptr || !SMX_SetLights2_ptr ||
		!SMX_ReenableAutoLights_ptr || !SMX_GetConfig_ptr || !SMX_SetConfig_ptr ||
		!SMX_FactoryReset_ptr || !SMX_ForceRecalibration_ptr || !SMX_SetTestMode_ptr ||
...
void SMX_Interface::SMXStateChangedCallback(int pad, SMXUpdateCallbackReason reason, void *pUser)
{
...
void SMX_Interface::Update_Light(smx_pad_light_map light, bool light_on)
{
	...
	if (light_on)
	{
		...
		for (int i = 0; i < BYTES_PER_PANEL; i += 3)
		{
			...
			if (light.smx_light_panel_offset < 9 && light.repeat_on_both)
			{
				...
				background->pattern[((NUM_OF_RGB_LEDS / 2) + offset) + (i + 0)] = light.r;
				background->pattern[((NUM_OF_RGB_LEDS / 2) + offset) + (i + 1)] = light.g;
				background->pattern[((NUM_OF_RGB_LEDS / 2) + offset) + (i + 2)] = light.b;
			}
		}
	}
	...
	{
		background->panel_on[light.smx_light_panel_offset + PANELS_PER_STAGE] = light_on;
	}
	...
	{
		background->panel_alpha[light.smx_light_panel_offset + PANELS_PER_STAGE] = MAX_BRIGHTNESS;
	}
...
void SMX_Interface::MakeOverlayMask()
{
	...
	for (int i = 0; i < NUM_OF_STAGES; i++)
	{
		...
		{
			int offset = (j * LEDS_PER_PANEL) + (i * (PANELS_PER_STAGE * LEDS_PER_PANEL));
...
uint8_t SMX_Interface::MixColors(uint8_t fgColor, float fgAlpha, uint8_t bgColor, float bgAlpha, float targetAlpha)
{
	return fgColor * (fgAlpha / targetAlpha) + bgColor * bgAlpha * ((1 - fgAlpha) / targetAlpha);
}
...
void SMX_Interface::CombineStates()
{
	for (int i = 0; i < NUM_OF_LEDS; i++)
	{
		...
		if (fg_pattern[i])
		{
			float res_alpha = 1 - (1 - background->panel_alpha[panelnum]) * (1 - foreground->panel_alpha[panelnum]);
			...
			else if (foreground->panel_alpha[panelnum] < MASK_FADE_INCREMENT)
			{
				...
				finalLightState[offset + 0] = background->pattern[offset + 0] * background->panel_alpha[panelnum];
				finalLightState[offset + 1] = background->pattern[offset + 1] * background->panel_alpha[panelnum];
				finalLightState[offset + 2] = background->pattern[offset + 2] * background->panel_alpha[panelnum];
			}
			else if (background->panel_alpha[panelnum] < MASK_FADE_INCREMENT)
			{
				// background is off, use the foreground
				finalLightState[offset + 0] = foreground->pattern[offset + 0] * foreground->panel_alpha[panelnum];
				finalLightState[offset + 1] = foreground->pattern[offset + 1] * foreground->panel_alpha[panelnum];
				finalLightState[offset + 2] = foreground->pattern[offset + 2] * foreground->panel_alpha[panelnum];
			}
			else
			{
				// mix the color.
				finalLightState[offset + 0] = MixColors(foreground->pattern[offset + 0], foreground->panel_alpha[panelnum], background->pattern[offset + 0], background->panel_alpha[panelnum], res_alpha);
				finalLightState[offset + 1] = MixColors(foreground->pattern[offset + 1], foreground->panel_alpha[panelnum], background->pattern[offset + 1], background->panel_alpha[panelnum], res_alpha);
				finalLightState[offset + 2] = MixColors(foreground->pattern[offset + 2], foreground->panel_alpha[panelnum], background->pattern[offset + 2], background->panel_alpha[panelnum], res_alpha);
			}
		}
		else
		{
