...
LightsDriverSextet::~LightsDriverSextet() { Disconnect(); };

bool LightsDriverSextet::ConnectHardware() {
  std::string devicePath = std::string("\\\\.\\") + IniHelper::Sextet.ComPort;

  // open device
  deviceHardware = CreateFileA(devicePath.c_str(), GENERIC_READ | GENERIC_WRITE,
...
bool LightsDriverSextet::ConnectPipe() {
  std::string pipePath =
      std::string("\\\\.\\pipe\\") + IniHelper::Sextet.PipeName;

  devicePipe = CreateFile(pipePath.c_str(), GENERIC_WRITE, 0, NULL,
...
bool LightsDriverSextet::PushTo(HANDLE device) {
  ...
  WriteFile(device, outputBuffer, FULL_SEXTET_COUNT, &bytes_written, NULL);
...
void LightsDriverSextet::Set(const LightsState *ls) {
  ...
  outputBuffer[0] |=
      (uint8_t)((ls->bass << 5) | (ls->bass << 4) |
                (ls->marquee_lr_right << 3) | (ls->marquee_lr_left << 2) |
                (ls->marquee_up_right << 1) | (ls->marquee_up_left << 0));

  outputBuffer[1] |= (uint8_t)((ls->p1_menu << 5) | (ls->p1_menu << 4));

  outputBuffer[3] |= (uint8_t)((ls->p1_down << 3) | (ls->p1_up << 2) |
                               (ls->p1_right << 1) | (ls->p1_left << 0));

  outputBuffer[7] |= (uint8_t)((ls->p2_menu << 5) | (ls->p2_menu << 4));

  outputBuffer[9] |= (uint8_t)((ls->p2_down << 3) | (ls->p2_up << 2) |
                               (ls->p2_right << 1) | (ls->p2_left << 0));
