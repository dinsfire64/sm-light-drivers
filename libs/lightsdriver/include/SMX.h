...
enum SMXUpdateCallbackReason {
  ...
  // disconnection, inputs changed, test data updated, etc.  It doesn't specify
  // what's changed.  We simply check the whole state.
  SMXUpdateCallback_Updated,

  // This is called when SMX_FactoryReset completes, indicating that
  // SMX_GetConfig will now return the reset configuration.
...
// thread-safe.
typedef void SMXUpdateCallback(int pad, SMXUpdateCallbackReason reason,
                               void *pUser);
...
// Update the lights.  Both pads are always updated together.  lightData is a
// list of 8-bit RGB colors, one for each LED.
//
// lightDataSize is the number of bytes in lightsData.  This should be 1350 (2
// pads * 9 panels * 25 lights * 3 RGB colors).  For backwards-compatibility,
// this can also be 864.
//
...
// 678 F01
//
// With 18 panels, 25 LEDs per panel and 3 bytes per LED, each light update has
// 1350 bytes of data.
//
...
//
// For backwards compatibility, if lightDataSize is 864, the old 4x4-only order
// is used, which simply omits lights 16-24.
...
// Set a sensor test mode and request test data.  This is used by the
// configuration tool.
SMX_API void SMX_SetTestMode(int pad, SensorTestMode mode);
SMX_API bool SMX_GetTestData(int pad, SMXSensorTestModeData *data);

// Set a panel test mode.  These only appear as debug lighting on the panel and
// don't return data to us.  Lights can't be updated while a panel test mode is
// active. This applies to all connected pads.
...
struct SMXInfo {
  ...
  // This device's serial number.  This can be used to distinguish devices from
  // each other if more than one is connected.  This is a null-terminated string
  // instead of a C++ string for C# marshalling.
  char m_Serial[33];
...
static_assert(sizeof(packed_sensor_settings_t) == 16,
              "Incorrect packed_sensor_settings_t size");
...
struct SMXConfig {
  // The firmware version of the master controller.  Where supported (version 2
  // and up), this will always read back the firmware version.  This will
  // default to 0xFF on version 1, and we'll always write 0xFF here so it
  // doesn't change on that firmware version.
  //
  ...
  // The version of this config packet.  This can be used by the firmware to
  // know which values have been filled in.  Any values not filled in will
  // always be 0xFF, which can be tested for, but that doesn't work for values
  // where 0xFF is a valid value.  This value is unrelated to the firmware
  // version, and just indicates which fields in this packet have been set. Note
  // that we don't need to increase this any time we add a field, only when it's
  ...
  // right, and Panel2 corresponds to UpLeft, UpRight, DownLeft and DownRight.
  // For later firmware versions, each panel is configured independently.
  //
  ...
  // Which sensors on each panel to enable.  This can be used to disable sensors
  // that we know aren't populated.  This is packed, with four sensors on two
  // pads per byte: enabledSensors[0] & 1 is the first sensor on the first pad,
  // and so on.
  uint8_t enabledSensors[5];

  // How long the master controller will wait for a lights command before
  // assuming the game has gone away and resume auto-lights.  This is in 128ms
  // units.
  uint8_t autoLightsTimeout = 1000 / 128; // 1 second

  // The color to use for each panel when auto-lighting in master mode.  This
  // doesn't apply when the pads are in autonomous lighting mode (no master),
  // since they don't store any configuration by themselves.  These colors
  // should be scaled to the 0-170 range.
  ...
  // The rotation of the panel, where 0 is the standard rotation, 1 means the
  // panel is rotated right 90 degrees, 2 is rotated 180 degrees, and 3 is
  // rotated 270 degrees. This value is unused.
...
static_assert(offsetof(SMXConfig, padding) == 201,
              "Incorrect padding alignment");
